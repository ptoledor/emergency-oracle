...
EMPTY_DAYS_LIMIT = 30  # dias vacios consecutivos sin rate limit = no hay mas tweets
...
async def scrape(args):
    ...
    async with async_playwright() as p:
        ...
        while True:
            pending = sorted([(s, u) for s, u in all_ranges if s not in done_periods], reverse=True)
            if not pending:
                break
            ...
            for i, (since, until) in enumerate(pending):
                ...
                if got_response:
                    ...
                    if len(rows) == 0 and not rate_limited:
                        consecutive_empty += 1
                    else:
                        consecutive_empty = 0
                    if consecutive_empty >= EMPTY_DAYS_LIMIT:
                        log.info(f"[+] {EMPTY_DAYS_LIMIT} dias vacios consecutivos — no hay mas tweets. Finalizando.")
                        break
                else:
                    done_periods.add(since)
                    recently_done.append(since)
